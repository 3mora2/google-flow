...
(generate, upscale, …) lives in :mod:`flow_cli.core.generator`.
"""
...
from flow_cli.utils.http import (
    HAS_CURL_CFFI,
    build_headers,
)
from flow_cli.utils.image import detect_mime_type
...
def _classify_error(exc: Exception) -> FlowError:
    ...
    if "timeout" in text:
        from flow_cli.exceptions import FlowTimeoutError
...
class FlowClient:
    ...
    async def upload_image(
        ...
        filename = f"flow_cli_upload_{int(time.time() * 1000)}.{ext}"
    ...
    async def upsample_image(
        ...
        *,
        ...
        if not encoded:
            from flow_cli.exceptions import FlowUpscaleError
